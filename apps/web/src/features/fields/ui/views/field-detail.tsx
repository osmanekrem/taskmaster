...
import {BooleanFormElement, StaticSelectOptionsFormElement,} from "@/components/type-form-element";
import {useEffect, useState} from "react";
import {queryClient} from "@/utils/trpc";
import {getSelectOptionsByFieldOptionIdsQuery} from "@/features/fields/lib/queries";
import {useMutation} from "@tanstack/react-query";
import {saveSelectOptionsMutation, updateFieldOptionValueMutation} from "@/features/fields/lib/mutations";
...
export default function FieldDetail() {
    ...
    const fetchSelectOptions = async (optionIds: string[]) => {
        return await queryClient.fetchQuery(getSelectOptionsByFieldOptionIdsQuery(optionIds));
    };
    ...
    useEffect(() => {
        ...
        if (data.options.some(
            (option) => option.key === "is-dynamic-options"
        )) {
            fetchSelectOptions(data.options.filter(
                (opt) => opt.key === "is-dynamic-options"
            ).map((opt: any) => opt.id)).then((res) => {
