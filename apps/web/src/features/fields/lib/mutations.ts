...
export const createFieldMutation = mutationOptions<
  CreateFieldResponseType,
  Error,
  CreateFieldRequestType
>({
  mutationFn: (data) => {
...
export const editFieldMutation = mutationOptions<
  EditFieldResponseType,
  Error,
  EditFieldRequestType
>({
  mutationFn: (data) => {
...
export const deleteFieldMutation = mutationOptions<
  DeleteFieldResponseType,
  Error,
  DeleteFieldRequestType
>({
  mutationFn: (data) => {
...
type UpdateFieldOptionValueResponse =
  RouterOutput['fields']['updateFieldOptionValue'];

export const updateFieldOptionValueMutation = mutationOptions<
  UpdateFieldOptionValueResponse,
  Error,
...
export const saveSelectOptionsMutation = mutationOptions<
  SaveSelectOptionsResponse,
  Error,
...
>({
  ...
  onSuccess: (res) => {
    queryClient.invalidateQueries({
      queryKey: ['select-options-by-field-option', res?.data[0]?.fieldOptionId],
    });
    queryClient.invalidateQueries({
      queryKey: ['field-with-details', res?.data[0]?.fieldId],
    });
    queryClient.invalidateQueries({
      queryKey: ['field-with-field-type', res?.data[0]?.fieldId],
    });
  },
});
