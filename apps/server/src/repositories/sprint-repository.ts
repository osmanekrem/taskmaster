...
export class SprintIssueRepository {
  ...
  async addIssue(
    ...
    if (finalPosition === undefined) {
      const [maxPos] = await db
        .select({
          max: sql<number>`COALESCE(MAX(${sprintIssues.position}), -1)`,
        })
        .from(sprintIssues)
  ...
  /**
   * Find sprint issue by issue ID (includes sprint details)
   */
  async findByIssueId(issueId: string) {
    return db.query.sprintIssues.findFirst({
      where: eq(sprintIssues.issueId, issueId),
      with: {
        sprint: {
          columns: { id: true, status: true, name: true, projectId: true },
        },
      },
    });
  }
    ...
    const [current] = await db
      ...
      .where(
        and(
          ...
          eq(sprintIssues.issueId, issueId),
        ),
      );
