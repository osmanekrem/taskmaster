...
export const issueComments = pgTable(
  'issue_comments',
  {
    ...
    parentId: text('parent_id').references((): any => issueComments.id, {
      onDelete: 'cascade',
    }),
    ...
    // Audit field (authorId serves as createdBy)
...
export const issueAttachments = pgTable(
  'issue_attachments',
  {
    ...
    // Audit field (uploaderId serves as createdBy)
    updatedBy: text('updated_by').references(() => user.id, {
      onDelete: 'set null',
    }),
  },
...
export const commentMentionRelations = relations(
  commentMentions,
  ({ one }) => ({
