...
export const issueLinkTypes = pgTable('issue_link_types', {
  ...
  isSystem: varchar('is_system', { length: 5 }).default('false'),
...
export const issueLinks = pgTable('issue_links', {
  ...
  // Link type
  linkTypeId: uuid('link_type_id').notNull().references(() => issueLinkTypes.id, { onDelete: 'restrict' }),
  ...
  unique('issue_links_unique').on(table.sourceIssueId, table.targetIssueId, table.linkTypeId),
]);
...
export const issueLinkTypesRelations = relations(issueLinkTypes, ({ many }) => ({
