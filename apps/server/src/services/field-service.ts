...
import {type CreateFieldSchema, type EditFieldSchema, type SelectOptionSchema} from "@/schemas/fields";

export const fieldService = (drizzle: typeof db | Parameters<Parameters<typeof db.transaction>[0]>[0] = db) => {
    ...
    return {
        ...
        createField: async (data: CreateFieldSchema) => {
            return await drizzle.transaction(async (tx) => {
                ...
                const relatedFieldTypeOptions = await repo.findFieldTypeOptionsByTypeId(newField.fieldTypeId);
        ...
        saveSelectOptions: async (fieldOptionId: string, options: SelectOptionSchema[]) => {
            return await drizzle.transaction(async (tx) => {
                const repo = fieldRepository(tx);
                ...
                await repo.deleteSelectOptionsNotInList(fieldOptionId, incomingOptionIds);
